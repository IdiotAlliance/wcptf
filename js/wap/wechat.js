	...
	for(var i=0;i<productarray.length;i++){
		if(productarray[i].sortid==sortid1){
			insert+='<li onclick=showitemdesc(this) data-value="'+productarray[i].productid+'" data-descexist="'+DESCNOEXIST+'">'+
		        ...
		        '<button class="btn-icon button-minus" data-value="'+productarray[i].productid+'" style="display:inline"><div class="img-in-btn" style="'+getbackground(MINUSBTN)+'"></div></button>'+
		        '<button class="btn-icon button-plus" data-value="'+productarray[i].productid+'" style="display:inline"><div class="img-in-btn" style="'+getbackground(PLUSBTN)+'"></button>'+
	        	'</div>'+
        		'</div>'+
        		'</li>';
            }
		...
		if(orderarray[i].count>0){
			...
			if(productindex>=0){
				insert+='<li>'+
		    		...
		    		'</div>'+
		        	'<div class="button-group-inlist" >'+
			        '<button class="btn-icon button-minus" data-value="'+orderarray[i].productid+'" style="display:inline"><div class="img-in-btn" style="'+getbackground(MINUSBTN)+'"></div></button>'+
			        '<button class="btn-icon button-plus" data-value="'+orderarray[i].productid+'" style="display:inline"><div class="img-in-btn" style="'+getbackground(PLUSBTN)+'"></div></button>'+
		        	'</div>'+
	        		'</div>'+
	        		'</li>';
        	}else{
...
function callsuccesspay(order){
	...
	if(order!=null){
		...
		if(order.poster_phone!=''){
			insert+='<br />送餐员电话：'+order.poster_phone;
		}
...
function initdesc(element1){
	...
	if(myproductindex>=0){
		var myproductdesc=productarray[myproductindex].productdesc!=null?productarray[myproductindex].productdesc:'该商品无详细描述';
		var insert='';
		insert+='<div class="product-item-desc">'+
			'<img class="myimg" onerror=hideself(this) src="'+BASEURL+productarray[myproductindex].productimg+'"alt="image">'+
			'<p>'+myproductdesc+'</p>'+
			'</div>';
