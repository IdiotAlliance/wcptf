...
class ProductTypeAR extends CActiveRecord
{
	...
	public function getProductsByType($storeId){
        ...
        $query = "select count(products_view.id) as product_count, product_type.id as typeId,
        		  product_type.type_name from product_type left join 
        		  (select * from products where products.deleted<>1) as products_view on products_view.type_id = product_type.id where product_type.store_id=:store_id and product_type.deleted = 0  group by product_type.id";
